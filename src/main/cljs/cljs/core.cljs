...
(defn- pr-opts []
  (->clj #js {:flush-on-newline *flush-on-newline*
...
(defn assoc
  ...
  ([coll k v]
   (if (implements? IAssociative coll)
     ...
     (if-not (nil? coll)
       (-assoc coll k v)
       (->clj (js-obj k v)) #_(array-map k v))))            ; Note: Only correct if k is string
  ([coll k v & kvs]
...
(defn iter [coll]
  (cond
    ...
    #_#_(nil? coll) (nil-iter)                              ; TODO this is not right
...
(extend-protocol IPrintWithWriter
  ...
  (-pr-writer [a writer opts]
    ...
    (pr-writer (->clj #js {:val (.-state a)}) writer opts)
    (-write writer "]"))
...
#_(def ^:private empty-map (->clj #js {}) #_(.. js/cljs -core -PersistentArrayMap -EMPTY)) ;; TODO fix

(defn- snapshot [x prop->key key->prop recursive?]
  x
  #_(let [result (volatile! (transient empty-map))]
    (gobject/forEach x (fn [v k _] (vswap! result assoc! (prop->key k)
                                     (cond-> v
...
(deftype ^:private TransientBean [^:mutable ^boolean editable?
  ...
  (-lookup [_ k not-found]
    (if editable?
      (let [ret (gobject/get obj (key->prop k) not-found)]
        (cond-> ret
  ...
  ITransientCollection
  (-conj! [tcoll o]
    (TransientArrayMap-conj! tcoll o editable?))
...
(deftype ^:private Bean [meta obj prop->key key->prop ^boolean recursive?
  ...
  ICollection
  (-conj [coll entry]
    (PersistentArrayMap-conj coll entry))
  ...
  (-assoc [_ k v]
    (if (snapshot? k v prop->key recursive?)
      ...
      (Bean. meta
        (doto (gobject/clone obj) (unchecked-set (key->prop k) (cond-> v recursive? unwrap)))
  ...
  IFind
  (-find [_ k]
    (let [v (gobject/get obj (key->prop k) lookup-sentinel)]
      (when-not (identical? v lookup-sentinel)
        (MapEntry. k (cond-> v recursive? (->val prop->key key->prop)) nil))))

  IMap
  (-dissoc [_ k]
    (Bean. meta (doto (gobject/clone obj) (js-delete (key->prop k)))
      prop->key key->prop recursive? nil nil nil))
  ...
  IEditableCollection
  (-as-transient [_]
    (TransientBean. true (gobject/clone obj) prop->key key->prop recursive? __cnt))
...
(deftype ^:private TransientArrayVector [^:mutable ^boolean editable?
  ...
  ITransientAssociative
  (-assoc! [tcoll key val]
    (TransientVector-assoc! tcoll key val "TransientArrayVector"))
...
(deftype ^:private ArrayVector [meta prop->key key->prop arr ^:mutable __hash]
  ...
  IEquiv
  (-equiv [coll other]
    (PersistentVector-equiv coll other ArrayVector (alength arr)))
  ...
  ILookup
  (-lookup [coll k] (-lookup coll k nil))
  (-lookup [coll k not-found] (PersistentVector-lookup coll k not-found))

  IAssociative
  (-assoc [coll k v]
    (PersistentVector-assoc coll k v))
  (-contains-key? [coll k]
    (PersistentVector-contains-key? coll k (alength arr)))
