...
;;; Note: The tests in this namespace manipulate native types
;;; (at test run time) and this namespace should be loaded last
;;; by test runners so as to not affect other tests.
...
(deftest test-empty-native
  (extend-type array
    IEmptyableCollection
    (-empty [coll] #js []))

  (let [v (empty #js [1 2 3])]
    (is (array? v))
    (is (zero? (count v)))))
