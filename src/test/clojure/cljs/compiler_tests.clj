...
(deftest test-cljs-2875
  (let [opts {}
    ...
    (testing "should replace with `then` branch"
      (is (= (emit '(if (str 1) true false))
             "true\n"))
      (is (= (emit '(if (inc 1) true false))
             "true\n"))
      (is (= (emit '(if (array) true false))
             "true\n"))
      (is (= (emit '(if (js-obj) true false))
             "true\n"))
      (is (= (emit '(if (fn []) true false))
             "true\n")))

    (testing "should replace with `else` branch"
      (is (= (emit '(if (prn) true false))
             "false\n")))
